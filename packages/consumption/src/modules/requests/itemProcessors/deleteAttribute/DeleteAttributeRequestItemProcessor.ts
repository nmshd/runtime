...
export class DeleteAttributeRequestItemProcessor extends GenericRequestItemProcessor<DeleteAttributeRequestItem> {
    public override async canCreateOutgoingRequestItem(requestItem: DeleteAttributeRequestItem, _request: Request, recipient?: CoreAddress): Promise<ValidationResult> {
        ...
        if (
            (attribute instanceof OwnIdentityAttribute && !(await this.consumptionController.attributes.isForwardedTo(attribute, recipient, true))) ||
            (attribute instanceof OwnRelationshipAttribute &&
                ((attribute.peer.equals(recipient) && attribute.isDeletedOrToBeDeletedByRecipient()) ||
                    (!attribute.peer.equals(recipient) && !(await this.consumptionController.attributes.isForwardedTo(attribute, recipient, true)))))
        ) {
            return ValidationResult.error(
                ConsumptionCoreErrors.requests.invalidRequestItem(
        ...
        if (attribute instanceof PeerRelationshipAttribute) {
            ...
            if (!(await this.consumptionController.attributes.isForwardedTo(attribute, recipient, true))) {
                return ValidationResult.error(
                    ConsumptionCoreErrors.requests.invalidRequestItem(
