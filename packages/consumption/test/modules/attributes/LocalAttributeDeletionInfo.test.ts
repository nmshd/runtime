...
import { ConsumptionController, LocalAttribute, LocalAttributeDeletionInfo, LocalAttributeDeletionInfoStatus } from "../../../src";
...
describe("LocalAttributeDeletionInfo", function () {
    ...
    describe("DeletionInfo of own shared Attributes", function () {
        test("should set the deletionInfo of an own shared Attribute to DeletionRequestSent", async function () {
            const deletionInfo = LocalAttributeDeletionInfo.from({ deletionStatus: LocalAttributeDeletionInfoStatus.DeletionRequestSent, deletionDate: CoreDate.utc() });
            ...
            expect(updatedOwnSharedIdentityAttribute!.deletionInfo!.deletionStatus).toBe(LocalAttributeDeletionInfoStatus.DeletionRequestSent);
        });

        test("should set the deletionInfo of an own shared Attribute to DeletionRequestRejected", async function () {
            const deletionInfo = LocalAttributeDeletionInfo.from({ deletionStatus: LocalAttributeDeletionInfoStatus.DeletionRequestRejected, deletionDate: CoreDate.utc() });

            ownSharedIdentityAttribute.setDeletionInfo(deletionInfo, testAccount.identity.address);
            await consumptionController.attributes.updateAttributeUnsafe(ownSharedIdentityAttribute);

            const updatedOwnSharedIdentityAttribute = await consumptionController.attributes.getLocalAttribute(ownSharedIdentityAttribute.id);
            expect(updatedOwnSharedIdentityAttribute!.deletionInfo!.deletionStatus).toBe(LocalAttributeDeletionInfoStatus.DeletionRequestRejected);
        });

        test("should set the deletionInfo of an own shared Attribute to ToBeDeletedByPeer", async function () {
            const deletionInfo = LocalAttributeDeletionInfo.from({ deletionStatus: LocalAttributeDeletionInfoStatus.ToBeDeletedByPeer, deletionDate: CoreDate.utc() });

            ownSharedIdentityAttribute.setDeletionInfo(deletionInfo, testAccount.identity.address);
            await consumptionController.attributes.updateAttributeUnsafe(ownSharedIdentityAttribute);

            const updatedOwnSharedIdentityAttribute = await consumptionController.attributes.getLocalAttribute(ownSharedIdentityAttribute.id);
            expect(updatedOwnSharedIdentityAttribute!.deletionInfo!.deletionStatus).toBe(LocalAttributeDeletionInfoStatus.ToBeDeletedByPeer);
        });

        test("should set the deletionInfo of an own shared Attribute to DeletedByPeer", async function () {
            const deletionInfo = LocalAttributeDeletionInfo.from({ deletionStatus: LocalAttributeDeletionInfoStatus.DeletedByPeer, deletionDate: CoreDate.utc() });

            ownSharedIdentityAttribute.setDeletionInfo(deletionInfo, testAccount.identity.address);
            await consumptionController.attributes.updateAttributeUnsafe(ownSharedIdentityAttribute);

            const updatedOwnSharedIdentityAttribute = await consumptionController.attributes.getLocalAttribute(ownSharedIdentityAttribute.id);
            expect(updatedOwnSharedIdentityAttribute!.deletionInfo!.deletionStatus).toBe(LocalAttributeDeletionInfoStatus.DeletedByPeer);
        });

        test("should throw trying to set the deletionInfo of an own shared Attribute to ToBeDeleted", function () {
            const deletionInfo = LocalAttributeDeletionInfo.from({ deletionStatus: LocalAttributeDeletionInfoStatus.ToBeDeleted, deletionDate: CoreDate.utc() });
        ...
        test("should throw trying to set the deletionInfo of an own shared Attribute to DeletedByOwner", function () {
            const deletionInfo = LocalAttributeDeletionInfo.from({ deletionStatus: LocalAttributeDeletionInfoStatus.DeletedByOwner, deletionDate: CoreDate.utc() });
    ...
    describe("DeletionInfo of peer shared Attributes", function () {
        test("should set the deletionInfo of a peer shared Attribute to ToBeDeleted", async function () {
            ...
            expect(updatedPeerSharedIdentityAttribute!.deletionInfo!.deletionStatus).toBe(LocalAttributeDeletionInfoStatus.ToBeDeleted);
        });
        ...
        test("should set the deletionInfo of a peer shared Attribute to DeletedByOwner", async function () {
            ...
            const updatedPeerSharedIdentityAttribute = await consumptionController.attributes.getLocalAttribute(peerSharedIdentityAttribute.id);
            expect(updatedPeerSharedIdentityAttribute!.deletionInfo!.deletionStatus).toBe(LocalAttributeDeletionInfoStatus.DeletedByOwner);
        });
    ...
    describe("DeletionInfo of third party owned RelationshipAttributes", function () {
        test("should set the deletionInfo of a third party owned RelationshipAttribute to DeletedByPeer", async function () {
            ...
            expect(updatedThirdPartyOwnedRelationshipAttribute!.deletionInfo!.deletionStatus).toBe(LocalAttributeDeletionInfoStatus.DeletedByPeer);
        });
