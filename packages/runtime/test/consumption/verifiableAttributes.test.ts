...
let unsignedAttribute: IdentityAttributeJSON;
let readAttributeRequest: CreateOutgoingRequestRequest;
...
beforeAll(async () => {
    ...
    unsignedAttribute = IdentityAttribute.from({
...
test.each(Object.values(SupportedVCTypes))("issue and present a credential of type %s", async (credentialType) => {
    ...
    expect(attributes[0].content.proof?.credentialType).toBe(credentialType);
...
test.each([
    ...
    setupMocks({ statusListCredential: revokedStatusCredential, statusListMediaType: mediaType });
...
test.each(Object.values(SupportedVCTypes))("don't trust an issuer with credential of type %s", async (credentialType) => {
    ...
    setupMocks({ issuerUntrusted: true });
...
function setupMocks(setupParams: { statusListCredential?: any; statusListMediaType?: string; issuerUntrusted?: boolean }) {
    ...
    handlers.push(
        http.get(issuerListUrl, () => {
            return new HttpResponse(null, {
                status: setupParams.issuerUntrusted ? 404 : 200
            });
        })
    );
    if (setupParams.statusListCredential && setupParams.statusListMediaType) {
        handlers.push(
            http.get(MOCK_STATUS_LIST_URI, () => {
                return (HttpResponse as any)[setupParams.statusListMediaType!](setupParams.statusListCredential);
            })
        );
    }
