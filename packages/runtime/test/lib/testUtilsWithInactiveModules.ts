import { IResponse, RelationshipCreationContentContainingResponse, RelationshipTemplateContentContainingRequestJSON, ResponseWrapperJSON } from "@nmshd/content";
...
export async function exchangeTemplateAndReceiverRequiresManualDecision(
    ...
    const request = (
        await rRuntimeServices.consumption.incomingRequests.received({
            receivedRequest: (template.content as RelationshipTemplateContentContainingRequestJSON).onNewRelationship,
