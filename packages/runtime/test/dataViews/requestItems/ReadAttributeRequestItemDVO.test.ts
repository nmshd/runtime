...
describe("ReadAttributeRequestItemDVO with IdentityAttributeQuery", () => {
    ...
    test("check the MessageDVO for the sender after acceptance", async () => {
        const baselineNumberOfAttributes = (await consumptionServices1.attributes.getAttributes({ query: { "content.value.@type": "GivenName", peer: address2 } })).value.length;
        ...
        const attributeResult = await consumptionServices1.attributes.getAttributes({
            query: { "content.value.@type": "GivenName", peer: dvo.request.peer.id }
        });
        expect(attributeResult).toBeSuccessful();
    ...
    test("check the MessageDVO for the recipient after they deleted the shared Attribute", async () => {
        ...
        await runtimeServices2.consumption.attributes.deleteAttributeAndNotify({ attributeId: sharedAttributeId });
    ...
    test("check the MessageDVO for the sender after they deleted the shared Attribute", async () => {
        ...
        await runtimeServices1.consumption.attributes.deleteAttributeAndNotify({ attributeId: sharedAttributeId });
...
describe("ReadAttributeRequestItemDVO with IQL and fallback", () => {
    ...
    test("check the MessageDVO for the sender after acceptance", async () => {
        const baselineNumberOfAttributes = (await consumptionServices1.attributes.getAttributes({ query: { "content.value.@type": "Surname", peer: address2 } })).value.length;
        const senderMessage = await exchangeAndAcceptRequestByMessage(runtimeServices1, runtimeServices2, requestContent, responseItems);
        const dto = senderMessage;
        const dvo = (await expander1.expandMessageDTO(senderMessage)) as RequestMessageDVO;
        expect(dvo).toBeDefined();
        ...
        const attributeResult = await consumptionServices1.attributes.getAttributes({
            query: { "content.value.@type": "Surname", peer: dvo.request.peer.id }
        });
        expect(attributeResult).toBeSuccessful();
        const numberOfAttributes = attributeResult.value.length;
        expect(numberOfAttributes - baselineNumberOfAttributes).toBe(1);
...
describe("AttributeSuccessionAcceptResponseItemDVO with IdentityAttributeQuery", () => {
    ...
    beforeEach(async () => {
        ...
        const { successor: successorOwnIdentityAttribute } = (
            await consumptionServices2.attributes.succeedOwnIdentityAttribute({
                predecessorId: predecessorOwnIdentityAttribute.id,
                successorContent: {
                    value: {
                        "@type": "GivenName",
    ...
    test("check the MessageDVO for the sender after acceptance", async () => {
        ...
        const predecessorResult = await consumptionServices1.attributes.getAttributes({
            query: { "content.value.@type": "GivenName", peer: dvo.request.peer.id, "content.tags": "x:predecessor" }
        });
        expect(predecessorResult).toBeSuccessful();
        expect(predecessorResult.value).toHaveLength(1);
        expect(predecessorResult.value[0].id).toBeDefined();
        ...
        const successorResult = await consumptionServices1.attributes.getAttributes({
            query: { "content.value.@type": "GivenName", peer: dvo.request.peer.id, "content.tags": "x:successor" }
        });
        expect(successorResult).toBeSuccessful();
        expect(successorResult.value).toHaveLength(1);
        expect(successorResult.value[0].id).toBeDefined();
...
describe("AttributeAlreadySharedAcceptResponseItemDVO with IdentityAttributeQuery", () => {
    ...
    beforeEach(async () => {
        const alreadySharedOwnIdentityAttribute = await executeFullCreateAndShareOwnIdentityAttributeFlow(runtimeServices2, runtimeServices1, {
            content: {
                value: {
                    "@type": "GivenName",
                    value: "aGivenName"
...
describe("ReadAttributeRequestItemDVO with RelationshipAttributeQuery", () => {
    beforeAll(async () => {
        const runtimeServices = await serviceProvider.launch(2, { enableRequestModule: true });
        runtimeServices1 = runtimeServices[0];
        runtimeServices2 = runtimeServices[1];
        ...
        const relationshipAttribute: RelationshipAttributeJSON = {
        ...
        responseItems = [{ accept: true, newAttribute: relationshipAttribute } as AcceptReadAttributeRequestItemParametersWithNewAttributeJSON];
    }, 30000);

    beforeEach(async () => await cleanupAttributes([runtimeServices2]));
    ...
    test("check the MessageDVO for the sender", async () => {
        ...
        expect(requestItemDVO.mustBeAccepted).toBe(true);
        expect(requestItemDVO.query).toBeDefined();
        expect(requestItemDVO.query.type).toBe("RelationshipAttributeQueryDVO");
        const relationshipAttributeQueryDVO = requestItemDVO.query as RelationshipAttributeQueryDVO;
        expect(relationshipAttributeQueryDVO.owner.id).toBe("");
    });
    ...
    test("check the MessageDVO for the recipient", async () => {
        ...
        expect(requestItemDVO.mustBeAccepted).toBe(true);
        expect(requestItemDVO.query).toBeDefined();
        expect(requestItemDVO.query.type).toBe("ProcessedRelationshipAttributeQueryDVO");
        const relationshipAttributeQueryDVO = requestItemDVO.query as ProcessedRelationshipAttributeQueryDVO;
        expect(relationshipAttributeQueryDVO.owner.id).toBe(address2);
    });
    ...
    test("check the MessageDVO for the recipient after acceptance", async () => {
        ...
        const recipientAddress = (await transportServices2.account.getIdentityInfo()).value.address;
        expect(responseItem.attribute.owner).toBe(recipientAddress);
        expect(responseItem.attribute.type).toBe("OwnRelationshipAttributeDVO");
        expect(responseItem.attribute.content.value["@type"]).toBe("ProprietaryString");
        expect((responseItem.attribute.content.value as GivenNameJSON).value).toBe("aString");
        expect(requestItemDVO.response).toStrictEqual(responseItem);
