...
import { FreeValueFieldTypes, FreeValueFormRequestItem } from "@nmshd/content";
import {
    ConsumptionServices,
...
describe("FreeValueFormRequestItemDVO", () => {
    beforeAll(async () => {
        ...
        responseItems = [{ accept: true, freeValue: "I accept to specify a free string value." } as AcceptFreeValueFormRequestItemParametersJSON];
    }, 30000);
    ...
    test("check the MessageDVO for the sender", async () => {
        ...
        expect(requestItemDVO.freeValueFieldType).toBe(FreeValueFieldTypes.TextField);
    ...
    test("check the MessageDVO for the recipient after acceptance", async () => {
        ...
        const responseItem = response!.content.items[0] as FreeValueFormAcceptResponseItemDVO;
        expect(responseItem.result).toBe("Accepted");
        expect(responseItem.type).toBe("FreeValueFormAcceptResponseItemDVO");
