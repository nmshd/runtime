...
import { SelectionFormRequestItem, SelectionFormRequestItemTypes } from "@nmshd/content";
import {
    ConsumptionServices,
...
describe("SelectionFormRequestItemDVO", () => {
    beforeAll(async () => {
        ...
        responseItems = [{ accept: true, options: ["optionA"] } as AcceptSelectionFormRequestItemParametersJSON];
    }, 30000);
    ...
    test("check the MessageDVO for the sender", async () => {
        ...
        expect(requestItemDVO.selectionType).toBe(SelectionFormRequestItemTypes.Radio);
    ...
    test("check the MessageDVO for the recipient after acceptance", async () => {
        ...
        const responseItem = response!.content.items[0] as SelectionFormAcceptResponseItemDVO;
