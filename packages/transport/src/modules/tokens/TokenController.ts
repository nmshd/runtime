...
export class TokenController extends TransportController {
    ...
    public async sendEmptyToken(parameters: ISendEmptyTokenParameters): Promise<EmptyToken> {
        ...
        const { hashedPassword, passwordProtection } = input.usePasswordProtection
            ? await this.generatePasswordProtection()
            : { hashedPassword: undefined, passwordProtection: undefined };
    ...
    private async generatePasswordProtection(): Promise<{ hashedPassword: string; passwordProtection: PasswordProtection }> {
