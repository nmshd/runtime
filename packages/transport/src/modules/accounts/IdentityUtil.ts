...
export class IdentityUtil {
    public static async createAddress(publicKey: ICryptoSignaturePublicKey, backboneBaseUrl: string): Promise<CoreAddress> {
        ...
        const checksumSource = CoreBuffer.fromUtf8(`${enmeshedPrefix}${backboneBaseUrl}:dids:${hashedPublicKey.toString(Encoding.Hex)}`);
        ...
        const addressString = `${enmeshedPrefix}${backboneBaseUrl}:dids:${identityPart}${checksum.toString(Encoding.Hex)}`;
        const addressObj = CoreAddress.from({ address: addressString });
        return addressObj;
    }

    public static async checkAddress(address: CoreAddress, backboneBaseUrl: string, publicKey?: ICryptoSignaturePublicKey): Promise<boolean> {
        const str = address.toString();

        const prefixLength = enmeshedPrefix.length;
        const strWithoutPrefix = str.substring(prefixLength);
        if (!strWithoutPrefix.startsWith(backboneBaseUrl)) {
            return false;
        }
