...
describe("RelationshipTemplateController", function () {
    ...
    function expectValidRelationshipTemplates(sentRelationshipTemplate: RelationshipTemplate, receivedRelationshipTemplate: RelationshipTemplate, nowMinusSeconds: CoreDate) {
        ...
        expect(sentRelationshipTemplate.createdAt.isSameOrAfter(nowMinusSeconds)).toBe(true);
        expect(receivedRelationshipTemplate.createdAt.isSameOrAfter(nowMinusSeconds)).toBe(true);
        expect(sentRelationshipTemplate.createdBy.toString()).toBe(receivedRelationshipTemplate.createdBy.toString());
        expect(sentRelationshipTemplate.identity.address.toString()).toBe(receivedRelationshipTemplate.identity.address.toString());
        expect(JSON.stringify(sentRelationshipTemplate.content)).toBe(JSON.stringify(receivedRelationshipTemplate.content));
    }
    ...
    describe("RelationshipTemplate deletion", function () {
        test("should delete own RelationshipTemplate locally and from the Backbone", async function () {
            const ownTemplate = await TestUtil.sendRelationshipTemplate(sender);
            const reference = ownTemplate.toRelationshipTemplateReference(sender.config.baseUrl);
        ...
        test("should delete a peer owned RelationshipTemplate locally", async function () {
            const ownTemplate = await TestUtil.sendRelationshipTemplate(sender);
            const reference = ownTemplate.toRelationshipTemplateReference(sender.config.baseUrl);
            const peerTemplate = await recipient.relationshipTemplates.loadPeerRelationshipTemplateByReference(reference);
