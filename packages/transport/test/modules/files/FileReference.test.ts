...
describe("FileReference", function () {
    test("should deserialize correctly (verbose, no version set)", async function () {
        const serialized = `{"@type":"FileReference","id":"${(await BackboneIds.file.generateUnsafe()).toString()}","key":${(await CryptoEncryption.generateKey()).serialize(false)}}`;
    ...
    test("should serialize and deserialize correctly (verbose)", async function () {
        ...
        expect(serialized).toBe(`{"@type":"FileReference","id":"${reference.id.toString()}","key":${reference.key.serialize(false)},"version":2}`);
        const deserialized = FileReference.deserialize(serialized);
        expect(deserialized).toBeInstanceOf(Serializable);
        expect(deserialized).toBeInstanceOf(FileReference);
        expect(deserialized.key).toBeInstanceOf(CryptoSecretKey);
