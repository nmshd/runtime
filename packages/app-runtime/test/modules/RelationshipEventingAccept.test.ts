...
describe("RelationshipEventingAcceptTest", function () {
    ...
    test("should fire events on the templator when relationship request was accepted (by templator itself)", async function () {
        ...
        expect(relationshipChangedEvent.data).toBeDefined();
        expect(relationshipChangedEvent.data.id).toBe(relationshipId);
        expect(relationshipChangedEvent.data.status).toBe(RelationshipStatus.Active);
    });

    test("should fire events on the requestor when relationship request was accepted", async function () {
